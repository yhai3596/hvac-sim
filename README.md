...
**v6**：智能实验助手——自然语言描述测试条件（例：「对比自动风和 2 档风的达温时间和除湿，从舒适度评价不同方案的差异」），解析为结构化实验计划（未提及变量取默认值、逐一修改变量做单变量对比），批量仿真（预热+测量、进度可视、可停止）后自动记入方案对比表并生成本地对比摘要与 LLM 分析报告；解析三路径：粘贴计划 JSON 直跑 / 已配置 API 轮询解析 / 内置规则识别常见对比（风量模式、算法、能力段、满足度、Tes、地点），另有「复制解析提示词」借助外部大模型的无 API 回退。
